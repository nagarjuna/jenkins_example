pipeline {
  ...
  stages {
    ...
    stage ('Run Unit tests'){
      steps {
        ...
        rvmSh "${ "export PORT=${env.TEST_PORT}" } ${ "CYPRESS_BASE_URL=http://localhost:${env.TEST_PORT} PORT=${env.TEST_PORT} yarn start-test 'start_test' 'http://localhost:${env.TEST_PORT}' cy:run" }"
      }
    }
  ...
  post {
      always {
        sh "export TMP_TEST_DB=${env.TMP_TEST_DB}"
        rvmSh "RAILS_ENV=test bundle exec rails db:drop"
      }
      // failure {
