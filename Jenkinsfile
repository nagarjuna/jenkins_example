  ...
  stages {
    ...
    stage ('Run Unit tests'){
      steps {
        ...
        rvmSh "${"PORT=${env.TEST_PORT}"}; PORT=${env.TEST_PORT} CYPRESS_baseUrl=http://localhost:${env.TEST_PORT} yarn start-test 'start_test' 'http://localhost:${env.TEST_PORT}' cy:run"
      }
    }
